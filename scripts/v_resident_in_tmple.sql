CREATE VIEW v_resident_in_temple (`name`, `dhamame`, `sex`, `type`, `kuti_number`, `kuti_type`, `arrive`, `leave`) AS
  SELECT `name`, `dhamame`, `sex`, `type`, `kuti_number`, `kuti_type`, `arrive`, `leave` FROM (
    (SELECT `id`, `name`, `dhamame`, `sex`, `type` FROM `tb_resident`) `tmp_resident`
    JOIN
    ...
    ON (`tmp_resident`.`id` = `tmp_resi_status`.`id`))